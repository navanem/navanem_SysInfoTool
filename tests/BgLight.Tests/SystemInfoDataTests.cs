...
namespace BgLight.Tests
{
    public class SystemInfoDataTests
    {
        ...
        public void Rows_returns_ordered_label_value_pairs()
        {
            ...
            Assert.Equal("User", rows[0].Label);
            Assert.Equal("Processor", rows[1].Label);
            Assert.Equal("Intel Core i7-1185G7", rows[1].Value);
            Assert.Equal("Serial No.", rows[2].Label);
            ...
            Assert.Contains(rows, r => r.Label == "Disk (C:)" && r.Value.Contains("400.0 Go") && r.Value.Contains("930.0 Go"));
            Assert.Contains(rows, r => r.Label == "Generated" && r.Value == "2026-06-22 10:00:00");
        }
    }
}
